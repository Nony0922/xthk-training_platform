...
系统共包含 **30 个功能模块**（含 AI 智能排课），按使用端与角色划分如下。
...
其他入口：请假在首页「功能服务」与「我的」页；课表 / 考勤 / 考试 / 成绩在首页「功能服务」。
...
| 端 | 模块 | 说明 |
|----|------|------|
| **小程序（8）** | 课程浏览/购买/订单、课程表、考勤、考试、成绩、**请假**、留言 | 家长端业务闭环；课表等按**学生分块表格**展示；购课含详情校验与**订单取消** |
...
- 页面：`course`（列表筛选 + 详情购课）、`order`（列表/详情，**取消待支付**）、`schedule`、`attendance`、`exam`、`score`、`message`、**`leave`**
...
| 后端关键点 | 登录扩展、`User` CRUD、`TeacherScopeService`、**`SparkAiService`**、`/schedule/ai/*` 编排 | `ParentApp`、**购课校验**、订单支付/取消、**`detectConflicts`** |
| 小程序 | 登录、首页、公告、个人中心 | 课程/订单、课表/考勤/考试/成绩（**按学生表格**）、**请假**、留言 |
...
5. **数据库升级**：若 `course` 表为旧版（缺少适用年级、授课方式等字段），请重新执行 `training.sql`，或执行该文件末尾注释中的 `ALTER TABLE course ...` 增量脚本
